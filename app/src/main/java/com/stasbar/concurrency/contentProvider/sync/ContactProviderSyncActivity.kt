...
class ContactProviderSyncActivity : ListActivity() {
    ...
    fun query(view: View? = null) {
        cr.acquireContentProviderClient(MyContentProvider.CONTENT_URI).query(MyContentProvider.CONTENT_URI, null, null, null, null).use {
            val items = ArrayList<Map<String, Any>>()
            while (it.moveToNext()) {
                val idIndex = it.getColumnIndex(BaseColumns._ID)
                val id = it.getInt(idIndex)
                val titleIndex = it.getColumnIndex(FeedReaderContract.FeedEntry.COLUMN_NAME_TITLE)
                val title = it.getString(titleIndex)
                val subTitleIndex = it.getColumnIndex(FeedReaderContract.FeedEntry.COLUMN_NAME_SUBTITLE)
                val subTitle = it.getString(subTitleIndex)
                val feed = Feed(id = id, title = title, subtitle = subTitle)
                items.add(mapOf(Pair(BaseColumns._ID, feed.id),
                        Pair(FeedReaderContract.FeedEntry.COLUMN_NAME_TITLE, feed.title)))
            }
            val columns = arrayOf(BaseColumns._ID, FeedReaderContract.FeedEntry.COLUMN_NAME_TITLE)
            val viewIds = intArrayOf(android.R.id.text1, android.R.id.text2)

            adapter = SimpleAdapter(applicationContext, items, android.R.layout.simple_list_item_2, columns, viewIds)
    ...
    fun delete(position: Int) {
        cr.acquireContentProviderClient(MyContentProvider.CONTENT_URI).use {
            ...
            it.delete(Uri.withAppendedPath(MyContentProvider.CONTENT_URI, id.toString()), null, null)
    ...
    fun update(position: Int) {
        cr.acquireContentProviderClient(MyContentProvider.CONTENT_URI).use {
            ...
            it.update(Uri.withAppendedPath(MyContentProvider.CONTENT_URI, id.toString()), values, null, null)
        }
